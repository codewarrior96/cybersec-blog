...
> This is the canonical English audit-grade README for AI auditors and detailed reviewers. For a Türkçe project overview (production demo link + brief Faz 1–5 summary + scope decisions), see [`../README.md`](../README.md).
>
> Moved from root to `docs/AUDIT_README.md` in Wave 9 (README hybrid restructure). All content below preserved verbatim from the Wave 7 capstone-grade English README, with relative cross-references updated for the new file location.
...
A namespace-by-namespace narrative lives in [`docs/audit/INDEX.md`](./audit/INDEX.md). Each row's full status text (including the closing-commit hash) lives in the phase audit doc itself.
...
- **R-API-03** (RLS asymmetry, Phase 3 RECLASSIFIED via Z.10): `platform-backbone-v1.sql` blueprint defines 21 tables with no RLS; production verification revealed those tables were **never applied to Supabase**. Future deployment cycle must ship RLS migration in the same operation. See Z.10 in [`docs/SCOPE_DECISIONS.md`](./SCOPE_DECISIONS.md).
...
- **7 npm audit vulnerabilities** (after Wave 6 patch-only fix): all require `--force` semver-major bumps (`next@16`, `react-simple-maps@1.0.0`). Documented per-package with production-bundle impact assessment in [`docs/audit/phase-1-a-final.md` § 8](./audit/phase-1-a-final.md#8-dependency-vulnerability-state-wave-6-housekeeping). Production bundle UNAFFECTED.

---

## Pattern catalog

Eighteen engineering patterns named, applied, or extended during the closure cycle. Full catalog: [`docs/PATTERN_CATALOG.md`](./PATTERN_CATALOG.md). Highlights:
...
1. This README.
2. [`docs/audit/INDEX.md`](./audit/INDEX.md) — single-page audit doc navigator.
3. [`docs/SCOPE_DECISIONS.md`](./SCOPE_DECISIONS.md) — Z.1..Z.13 mentor decisions consolidated.
4. [`docs/PATTERN_CATALOG.md`](./PATTERN_CATALOG.md) — 18 engineering patterns with instance lineage.
5. Phase audit docs in order:
   - [`docs/audit/phase-1-a-final.md`](./audit/phase-1-a-final.md) — Security & identity (R-01..R-22, plus § 8 npm audit state).
   - [`docs/audit/phase-2-a-lab-engine-audit.md`](./audit/phase-2-a-lab-engine-audit.md) — Lab Engine (R-LAB-01..15).
   - [`docs/audit/phase-3-a-api-contracts-audit.md`](./audit/phase-3-a-api-contracts-audit.md) — API contracts (R-API-01..15).
   - [`docs/audit/phase-4-a-ui-a11y-audit.md`](./audit/phase-4-a-ui-a11y-audit.md) — UI & a11y (R-UI-01..15).
   - [`docs/audit/phase-5-a-e2e-journeys-audit.md`](./audit/phase-5-a-e2e-journeys-audit.md) — E2E journeys (R-E2E-01..13).
6. [`docs/audit/phase-1-a-pending-amendments.md`](./audit/phase-1-a-pending-amendments.md) — A-01..A-21 amendment ledger.
...
- [`docs/platform-backbone-plan.md`](./platform-backbone-plan.md) — Supabase Postgres backbone narrative.
- [`docs/data-flow-map-and-migration-plan.md`](./data-flow-map-and-migration-plan.md) — domain-by-domain data flow audit.
- [`docs/postgres-migration-execution-roadmap.md`](./postgres-migration-execution-roadmap.md) — Phase 1+ migration steps.
