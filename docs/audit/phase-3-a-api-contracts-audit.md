...
| R-API-07 ✅ RESOLVED (Wave 5B) | Medium | A04 Insecure Design | `src/app/api/profile/certifications/route.ts` (POST), `src/lib/portfolio-assets.ts:saveCertificationAsset` | **File upload validation**: `assertMagicMatches` (portfolio-assets.ts L70) checks magic bytes for JPEG/PNG/WEBP/PDF. MAX_CERTIFICATION_ASSET_BYTES = 10 MB. Filename sanitization replaces `[^a-zA-Z0-9._-]` with `-`. BUT: no quota per user — a single user can upload 1000s of certifications, each up to 10 MB, accumulating storage. No content-type sniff beyond magic bytes (file could be valid PDF wrapper containing JS). No virus scanning. | Storage exhaustion: attacker creates account, uploads 1000 × 10 MB PDFs. 10 GB consumed before any cap fires. Supabase Storage bucket fills. Severity Medium because (a) requires authenticated user, (b) Supabase bucket sizing operational issue, (c) realistic in demo context with no monitoring. Also: PDF content not scanned (a real PDF can carry JavaScript that executes when an admin downloads + opens via desktop viewer). **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — new `src/lib/quota.ts` centralizes per-user count caps (`MAX_CERTIFICATIONS_PER_USER = 20`, `MAX_EDUCATION_PER_USER = 20`) + per-asset size caps (re-export of `MAX_CERTIFICATION_ASSET_BYTES = 10 MB`, `MAX_AVATAR_ASSET_BYTES = 5 MB` for discoverability). `checkCountQuota` gate wired into `certifications` POST BEFORE asset upload so over-quota users never trigger storage I/O. `quotaReasonToStatus` maps `count_exceeded → 429` + `size_exceeded → 413` per RFC 7231. T-QC01-04 (4 tests in `src/lib/quota.test.ts`) lock the gate semantics + policy constants. Virus scanning + per-asset content sniffing remain Phase 6 operational decisions (rejected alternative in source). |
| R-API-08 ✅ RESOLVED (Wave 5B) | Medium | A06 Vulnerable & Outdated Components | `src/app/api/cybernews/route.ts` (5 RSS feeds, regex-based XML parser) | **Custom XML parsing via regex** (cybernews route L37-49 `extractTag` uses `RegExp` with hand-rolled CDATA handling). Regex-based XML parsing is a known anti-pattern: external feed content can include nested tags, malformed CDATA, or unicode that breaks parser assumptions. The 5 feeds (THN, Krebs, BleepingComputer, SANS ISC, SecurityWeek) are public RSS aggregators; if one feed is compromised + serves malicious XML, parser behavior is undefined. Additional risk: `cleanText` decodes HTML entities including `<` and `>` from feed content — combined with feed.description copied into client UI, a malicious feed could embed scripts that round-trip through the parser-decoder. | Compromise scenario: attacker MITM's one of the 5 feed URLs (e.g., via DNS hijack of feedburner.com), serves malicious RSS. The cleanText decoder + 220-char-truncated description ends up in the UI. If UI renders descriptions as HTML, stored-XSS via aggregator content. Severity Medium because (a) requires upstream feed compromise, (b) UI render path determines exploit (not in Phase 3 scope), (c) Phase 1 R-13 closed the email side of similar threat — UI surface deferred to Phase 4 (UI & Accessibility) per CLAUDE.md L172. **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — hand-rolled regex `extractTag` / `extractLink` removed; route now uses `fast-xml-parser` (v5.8.0) with `ignoreAttributes: false`, `cdataPropName: '__cdata'`, `trimValues: false` config (preserves attribute data needed for Atom `<link href>` + content fidelity for downstream `cleanText`). New helpers `extractTextNode` / `extractLinkHref` / `extractItemLink` normalize parser output across RSS 2.0, Atom (`<link href>`), and FeedBurner-wrapped feed shapes. Malformed XML returns `[]` from `parseFeed` (defensive — no throw propagates to the outer `Promise.allSettled`). T-CN01-04 (4 tests in `src/app/api/cybernews/__tests__/cybernews.test.ts`) lock RSS 2.0 + Atom + malformed-XML + CDATA/entity-decoding paths. |
| R-API-09 ✅ RESOLVED (Wave 5B) | Medium | A04 Insecure Design + A09 | `src/app/api/cves/route.ts`, `src/app/api/greynoise/route.ts` | **No rate limiting on external-call routes**. /api/cves makes one NVD API call per request (5 req/30s NVD limit). /api/greynoise makes one Greynoise API call. Both have `s-maxage=300` server cache, BUT cache is per-Vercel-edge-node not global; with many concurrent fresh requests, edge cache stampede possible. /api/cybernews fans out 5 feed fetches per request. No rate limit on any of the three. Combined: a malicious user can drive 5 cybernews-feed fetches × N concurrent requests = bandwidth amplification. | Attacker scripts 100 concurrent fetches to /api/cybernews → 500 outbound RSS fetches before edge cache catches up. NVD anonymous rate limit (5 req/30s) trips quickly under similar amplification on /api/cves, then /api/cves returns 500 to legitimate users for ~30s. Severity Medium because (a) no auth on these routes means anonymous DoS possible, (b) impact bounded by external rate limits + server cache, (c) Phase 1.5.9 R-02 hardened the auth-route rate limiter but didn't extend to external-call routes. **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — all three external routes (cves, greynoise, cybernews) now gate on `checkRateLimit` BEFORE any upstream call using the existing Phase 1.5.9 rate-limiter infrastructure. Uniform `RATE_LIMIT_OPTIONS = { max: 60, windowMs: 60_000 }` per route (per-IP bucket; cybernews bucket name = `api:external:cybernews` etc.). Limited requests short-circuit to 429 + `Retry-After` header — no upstream fetch, no `recordFailure` budget burn. T-EX01-03 (3 tests in `src/app/api/__tests__/external-rate-limit.test.ts`) lock the uniform gate semantics across all three surfaces. |
| R-API-10 ✅ RESOLVED (Wave 5B) | Medium | A05 Security Misconfiguration | `src/app/api/profile/avatar/[userId]/route.ts` GET | **Avatar GET allows authenticated cross-user fetch**: route at L29 reads `userId` from URL path, returns ANY user's avatar to any authenticated session — no ownership gate. This is correct for "everyone needs to see operator avatars in attribution columns" UX, but the GET also returns a Supabase **signed URL** (L48 `createSignedObjectUrl(avatarMeta.assetPath, 60)`) with 60-second TTL — which the recipient can share or pin in browser tab. Cross-user signed URLs persist for 60s after the original request. | Limited blast radius: avatars are user-uploaded portrait images, not sensitive. But a signed URL leaked off-platform (clipboard share, screenshot of URL bar) can be retried by anyone for 60s. Severity Medium because (a) signed-URL leak via referrer/share is a real-world pattern, (b) 60s window is operationally short, (c) the trade-off between "everyone sees attribution avatars" UX and "signed URL never shared" is a Phase 3 doc-level decision, not a bug. **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — `createSignedObjectUrl(avatarMeta.assetPath, 15)` — TTL tightened from 60s to 15s. 15s still accommodates network jitter + browser navigation but makes off-platform reuse window narrow enough to be operationally negligible. REJECTED ALTERNATIVE: 5s — rejected as too aggressive for slow mobile networks (legitimate clients would race the TTL). T-AV-TTL (1 test in `src/app/api/profile/avatar/__tests__/avatar.test.ts`) locks the 15s invariant — any drift back to 60s fails the test. |
| R-API-11 ✅ RESOLVED (Wave 5B) | Low | A09 | `src/app/api/profile/avatar/route.ts` (POST) | **Avatar upload cleanup on partial failure**: route at L26-57 uploads new asset → calls updatePortfolioAvatar → on `!updated`, deletes the new asset (L42) and returns 404. On any thrown exception (L51-57), deletes new asset and returns 400. Cleanup is best-effort. If `deleteStoredAsset` itself throws (Supabase Storage outage), the new asset stays orphaned. No retry queue, no orphan-sweep batch. | Storage build-up over time as failed uploads leave orphans. Severity Low because (a) requires partial-failure path to trigger, (b) avatar files are small (≤5 MB), (c) no security implication. **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — previous-avatar cleanup wrapped in try/catch. Cleanup failure is logged via `console.warn` (orphan tracking — userId + oldAssetPath + error message) but does NOT propagate: the user receives a 200 response with the new avatar already committed; the OLD asset becomes an orphan for a future cron sweep (Phase 6). Prior bug: cleanup throw rolled back the new-avatar upload via the outer catch block, even though the avatar record had already been successfully updated in DB. T-AV-ORPHAN01-02 (2 tests in `src/app/api/profile/avatar/__tests__/avatar.test.ts`) lock the best-effort behavior + the happy-path cleanup. Cron-based orphan-sweep batch deferred to Phase 6 (operational concern, not security). |
| R-API-12 ✅ RESOLVED (Wave 5B) | Low | A04 Insecure Design | `src/app/api/reports/route.ts` (POST `severity` field default `'LOW'` when unspecified) | **Severity taxonomy ambiguity**: reports route uses uppercase 4-level `'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'`. attack_events table (per `supabase/attack-events.sql:9`) uses lowercase 3-level `'critical' | 'high' | 'low'`. live-attacks route maps `'P1' → 'critical'`, etc. Dashboard (per Phase 1.5.14 dashboard tuning audit Section E) uses uppercase 4-level `'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW'`. **Three parallel non-communicating severity taxonomies** in the codebase. | Latent risk — currently no bridge between layers. If Phase 3 introduces an integration (e.g., dashboard subscribes to attack_events table), normalization is required. Severity Low because no current exploit, just documentation/integration hygiene concern. **STATUS (Wave 5B commit `bb488c6`):** RESOLVED — new `src/lib/severity-taxonomy.ts` flattens the 3 in-tree taxonomies into a canonical 5-level lowercase form (`critical \| high \| medium \| low \| info`) with `parseCanonicalSeverity` (strict, returns null) + `normalizeToCanonicalSeverity` (lenient, returns 'info' fallback) + `canonicalSeverityWeight` (5-tier sort weight) + `canonicalToReportSeverity` (back-map for the reports surface). Wired into `reports/route.ts` POST (parseSeverity refactored to delegate; 'info' input still rejected to preserve the prior 400-on-unknown contract) + `live-attacks/route.ts` GET (additive `canonicalSeverity` field on AttackEvent — non-breaking; existing 3-level `severity` retained). The existing `normalizeSeverity` in `soc-attack-utils.ts` (AttackSeverity-typed) is UNCHANGED — different name, different return type, no collision. T-ST01-08 (8 tests in `src/lib/severity-taxonomy.test.ts`) cover strict parse + lenient normalize + alias inputs (crit/med/informational) + 5-level weight ordering + frozen-array invariant + reports back-mapping + round-trip from all 3 in-tree taxonomies. |
...
| R-API-14 ✅ RESOLVED (Wave 5C) | Low | A04 Insecure Design | `src/app/api/profile/certifications/[id]/route.ts` (DELETE), `education/[id]/route.ts` (DELETE) | **DELETE permanently removes records** — no archive intermediate state. Compare with `reports/[id]/route.ts:14-19` two-stage delete (archive then permanent). Certifications + educations: one-click delete is irreversible. UX inconsistency + accidental-delete risk. | User clicks "delete cert" → record gone, asset binary deleted from Supabase Storage. No recovery. Severity Low because (a) intentional UX choice (per code comment absent), (b) no security implication, (c) educational portfolio context tolerates this. Noted as design-consistency observation, not bug. **STATUS (Wave 5B commit `bb488c6`):** DEFERRED to Wave 5C. Mid-cycle state-gathering surfaced that mirroring the reports two-stage archive pattern requires adapter schema changes (cert/edu records have no `archivedAt` field; would require new adapter method + memory store + Supabase store updates). Per Wave 5B yasaklar "If reports archive pattern is fundamentally different from mirror plan — STOP, report, mentor decides", scope is intentionally bounded to the 6 closures (R-API-07/08/09/10/11/12) that fit within Wave 5B's budget. R-API-14 closure carries forward to Wave 5C with the schema-extension work explicitly in scope. **STATUS (Wave 5C commit `<COMMIT_HASH_TBD>`):** RESOLVED — `archivedAt: string \| null` field added to `PortfolioCertificationRecord` + `PortfolioEducationRecord` (portfolio-profile.ts) + parallel `InternalCertification`/`InternalEducation` (soc-store-memory.ts). New adapter methods `archivePortfolioCertification` + `archivePortfolioEducation` wired through Supabase JSON branch + memory/sqlite fallback in soc-store-adapter.ts. Memory store + Supabase Storage JSON store fully implement archive (sets archivedAt + audit log) + NOT_ARCHIVED guard on delete. Backward-compat: Supabase store reads normalize missing-field → null for legacy records. PATCH `/api/profile/{certifications,education}/[id]?action=archive` query-param dispatch (mirrors the reports archive flow with a routing tweak so existing PATCH form-body bodies stay intact). DELETE rejects non-archived records with 409 Conflict. Sqlite fallback path: ALTER TABLE deferred (Z.10 + Z.11 constraint); synthesized `archivedAt: null` on read means sqlite-mode DELETE is effectively delete-blocked — acceptable degradation; production runs Supabase Storage JSON which carries the field properly. T-CA01-04 (4 cert tests) + T-EA01-04 (4 edu tests) lock the contract: archive sets archivedAt + 200; DELETE non-archived → 409; DELETE archived → 200; archive on missing id → 404. |
...
**Summary by severity (Phase 3.D + Wave 1 + Wave 2B + Wave 3 + Wave 5B + Wave 5C updates marked):** High = 4 (R-API-01 CLOSED via T-PC01-20, R-API-02 CLOSED via T-AL01-20, **R-API-03 🟡 RECLASSIFIED** — RLS deferred per Z.10, **R-API-04 ✅ RESOLVED Wave 3** via T-UD01-08 partial→full closure); Medium = 6 (**R-API-05 ✅ FIXED** via sanitize.ts + T-RP07-13, **R-API-06 ✅ RESOLVED Wave 2B** via requireRole analyst gate + T-UR01-04, **R-API-07 ✅ RESOLVED Wave 5B** via quota.ts + T-QC01-04, **R-API-08 ✅ RESOLVED Wave 5B** via fast-xml-parser + T-CN01-04, **R-API-09 ✅ RESOLVED Wave 5B** via external rate-limit gate + T-EX01-03, **R-API-10 ✅ RESOLVED Wave 5B** via 60s→15s TTL + T-AV-TTL); Low = 4 (**R-API-11 ✅ RESOLVED Wave 5B** via best-effort cleanup + T-AV-ORPHAN01-02, **R-API-12 ✅ RESOLVED Wave 5B** via severity-taxonomy.ts + T-ST01-08, **R-API-13 ✅ RESOLVED Wave 2B** via 6th defense-in-depth instance + T-PM01-08, **R-API-14 ✅ RESOLVED Wave 5C** via archivedAt schema extension + PATCH-archive + DELETE-purge + T-CA01-04 + T-EA01-04); Informational = 1 (**R-API-15 ✅ DOC-ONLY closure Wave 1**). **Total = 15. R-API-RESOLVED count: 14 of 15 (93%) + 1 reclassified (R-API-03 non-actionable per Z.10). R-API namespace effective complete from capstone scope perspective.**
...
**Resolution (Wave 5B commit `bb488c6`):** CLOSED — earlier deferral lifted. New `src/lib/severity-taxonomy.ts` provides a canonical 5-level lowercase normalizer (`critical | high | medium | low | info`) plus strict + lenient variants, weight ordering, and a reports-back-mapper. Wired into both reports POST (refactored `parseSeverity`) and live-attacks GET (additive `canonicalSeverity` field; non-breaking). 8 unit tests T-ST01-08 (in `src/lib/severity-taxonomy.test.ts`) lock the contract. Existing `normalizeSeverity()` in `soc-attack-utils.ts` left untouched (different name, different return type — no collision; consumers of the 3-level AttackSeverity flow unchanged).
