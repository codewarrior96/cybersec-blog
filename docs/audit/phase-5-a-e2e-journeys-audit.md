...
|---|---|---|---|---|---|
| R-E2E-01 🟡 PARTIAL CLOSURE (Wave 4B) | **Critical** | A07 ID&A Failures | `/register` → `/auth/verify-pending` → `/verify` → `/login` → `/home` (full auth bootstrap) | **Auth bootstrap end-to-end untested.** Phase 1.D covers each route handler in isolation (~85 tests across login/register/logout/session/forgot/reset/verify/verify-resend), and Phase 1.5 hardened individual primitives (scrypt, rate-limiter, identity-validation). But the **glue between routes** — the user actually completing the register → email-verify → login → land-on-home sequence — has no test coverage. Production bug here = no one can sign up = demo-blocking. | Real attack chain risk: a regression to the verification token validation (e.g., URL-encoding mismatch between email template and verify endpoint) could silently break sign-up while individual unit tests stay green. Severity Critical because (a) it's the entire user-acquisition funnel, (b) no other test phase covers the cross-route handoff, (c) Resend email delivery is in the loop (production-only verifiable). **STATUS (Wave 4B commit `<COMMIT_HASH_TBD>`):** PARTIAL CLOSURE per Yol A (Phase 5.A Z.12). `e2e/journey-auth-bootstrap.spec.ts` (T-E1-01..07, 6 active + 1 skip): /login form surface (T-E1-01), /register 5-field surface (T-E1-02), ephemeral register submission lands on /auth/verify-pending (T-E1-03, full network round-trip against production), duplicate-credential rejection (T-E1-05, locks emailKey uniqueness contract), invalid-login generic-error contract (T-E1-06, A-14/R-22 no-enumeration assertion), /home anonymous → inline EmbeddedLogin (T-E1-07, HomePageClient unauth path). T-E1-04 (verification-link click) SKIPPED with explicit Phase 6 operational dependency note — Yol A declined Resend sandbox / pre-verified user / SERVICE_ROLE_KEY paths. Full closure deferred to Phase 6 operational. |
| R-E2E-02 🟡 PARTIAL CLOSURE (Wave 4B) | **High** | A04 Insecure Design + WCAG 2.1.1 Keyboard | `/community` (Terminal + Lab Engine integration) | **Lab L1 happy-path solve untested end-to-end.** Phase 2.D covers Lab Engine logic (76 tests across validation/contract, reveal/detector, mutation/operations, filesystem, ctf-regression, cross-context-bypass). Phase 4.D covers AnsiText pure parser (16 tests). But **Terminal.tsx render layer + the user actually typing commands + reveal banner appearing + flag submission** is unverified together. Phase 4.A explicitly routed Terminal.tsx (861 LOC, R-UI-01) to Phase 5 E2E. | Flagship demo feature. A regression to Terminal's input handling (e.g., key binding for Tab completion, scroll-back retention) or to the engine→Terminal callback wiring would break the visible UX while engine-only tests stay green. Severity High because (a) it's the central interview-demo path, (b) unit tests can't catch the wiring layer, (c) accessibility (R-UI-01 partial) needs real-browser keyboard + screen-reader coverage. **STATUS (Wave 4B commit `<COMMIT_HASH_TBD>`):** PARTIAL CLOSURE per Yol A (Phase 5.A Z.12). Wave 4B state gathering surfaced that `src/app/community/layout.tsx` (BUG-006 closure) server-side gates `/community` via `redirect('/login')` for anonymous sessions — initial state gathering only inspected `page.tsx` and missed the layout. `e2e/journey-lab-l1-solve.spec.ts`: T-E2-01 ACTIVE (anon → /login redirect contract locked), T-E2-02..05 SKIPPED with explicit Yol A reason — ephemeral unverified user cannot log in (403 EMAIL_NOT_VERIFIED) → cannot reach Terminal mount path. Lab Engine business logic remains fully covered at unit level via Phase 2.D 76 tests; the gap this closure leaves is BROWSER INTEGRATION (Terminal render + keystroke → output) which depends on verified-user availability. R-UI-01 status preserved as PARTIAL pending Phase 6 / Wave 5 verified-user setup. |
| R-E2E-03 🟡 PARTIAL CLOSURE (Wave 4B) | **High** | A01 Broken Access Control + persistence | `/portfolio?tab=certifications` (CRUD + R-API-01 idioms) | **Portfolio certification CRUD round-trip untested.** Phase 3.D Target #1 covers route handler IDOR closure (T-PC01-20, 20 tests). Phase 4.A routed PortfolioWorkspace (1564 LOC) to Phase 5. The user actually filling the form, submitting, navigating away, returning, and seeing the certification still present is the persistence contract — which depends on `SOC_IDENTITY_STORE=supabase` correctly persisting via Supabase Storage JSON (Z.10 critical assumption). | Production-realistic threat: if Storage JSON write succeeds but read shape drifts, the certification "saves" but doesn't appear on reload. Unit tests can't catch this because they mock the adapter. Severity High because (a) it's the user's portfolio data — losing it is demo-killing, (b) it's the simplest persistence verification that exercises Supabase Storage JSON read+write round-trip on production. **STATUS (Wave 4B commit `<COMMIT_HASH_TBD>`):** PARTIAL CLOSURE per Yol A (Phase 5.A Z.12). `e2e/journey-portfolio-cert-crud.spec.ts`: T-E3-01 + T-E3-02 ACTIVE (anon → /login redirect contract; tab=certifications param doesn't bypass the gate), T-E3-03..06 SKIPPED with Yol A reason — auth-gated `/portfolio` page requires verified session ephemeral user cannot obtain. The Z.11 Supabase Storage JSON persistence verification — the most operationally important assertion of R-E2E-03 — remains pending verified-user setup (Phase 6). Phase 3.D T-PC01-20 covers API contracts at unit level. |
...
Phase 5.D commit `b1cddcd` will implement Targets #1-3 above. Specific test IDs (T-E1-01..T-E3-06) locked per Section 9 Z.X decisions.

---

## 6. Phase 5.B Infrastructure Needs

**Status (Wave 4A — Phase 5.B + 5.C atomic per Z.2 cadence):** SHIPPED in commit `b1cddcd`. `@playwright/test@^1.60.0` installed; Chromium binary detection succeeded (chromium-1223 cached at `%LOCALAPPDATA%/ms-playwright/` from prior Playwright work on the dev workstation — `npx playwright install chromium` was effectively a no-op for this dev machine; CI runners trigger fresh download via the workflow's `npx playwright install chromium --with-deps` step). `playwright.config.ts` at repo root: `testDir './e2e'`, `baseURL` env-driven (defaults to `https://siberlab.dev` per Z.10), Chromium-only project per Z.3, CI-aware retries/workers/reporter, trace/screenshot/video retain-on-failure. `.gitignore` extended with E2E artifact paths (test-results, playwright-report, blob-report, playwright/.cache, e2e/_storage/*.json, .env.test.local). E2E fixtures (`e2e/_fixtures/`) — `users.ts` (ephemeral random factory per Z.5), `resend-mock.ts` (page.route() interception per Z.4 with documented server-side limitation), `auth.ts` (registerAndLogin helper with placeholder selectors pending Wave 4B confirmation), barrel `index.ts`. `e2e/_storage/.gitkeep` for hybrid storageState directory (Z.6). GitHub Actions workflow `.github/workflows/e2e.yml` shipped with `workflow_dispatch`-only gate; operator removes the gate after Wave 4B green run. `package.json` scripts added: `e2e`, `e2e:headed`, `e2e:ui`, `e2e:install`. **Smoke validation passed** (5.5s test against production siberlab.dev `/` → title matches /siberlab/i) then smoke file DELETED before commit; baseline 489 / 55 preserved (Vitest unaffected — separate runner). **Operator post-push action required:** add `TEST_USER_PASSWORD` to GitHub Actions Secrets (Settings → Secrets and variables → Actions). Phase 5.D (Wave 4B) test cases follow in next cycle.
...
### Z.13 — Yol A pragmatic fallback (added Wave 4B / Phase 5.D)

Operator decision during Wave 4B kickoff: declined the three paths that would enable full verification flow coverage:
- **Resend sandbox** — would intercept test email dispatch + return verification URLs without sending real mail. Operational complexity rejected.
- **Pre-verified test user** — operator manually creates + verifies one user; test runs reuse known credentials in CI secret. Operational maintenance rejected.
- **SUPABASE_SERVICE_ROLE_KEY in test env** — would allow direct DB token extraction. Rejected per security policy (no production secrets in test runner).

**Consequence (locked in Wave 4B commit):** Wave 4B ships:
- R-E2E-01 **PARTIAL CLOSURE** — 6 active + 1 documented skip (T-E1-04 verification-link path)
- R-E2E-02 **PARTIAL CLOSURE** — 1 active (anon redirect) + 4 skipped (Terminal mount + Lab solve require verified user)
- R-E2E-03 **PARTIAL CLOSURE** — 2 active (anon redirect both with/without tab param) + 4 skipped (CRUD requires verified user)

State gathering during Wave 4B execution discovered that `/community` is also server-side auth-gated (via `src/app/community/layout.tsx` BUG-006 closure, not just `/portfolio`). Initial Phase 5.A audit doc inventory only inspected `page.tsx` files and missed the layout-level gate.

**Honest closure signal preserved:** all skips have explicit `test.skip(...)` calls with inline reason citing Phase 6 operational dependency. AI auditor sees scoped + documented coverage, not "claimed but not actually exercised."

**Phase 6 unblocks** when operator chooses one of the three paths above. The skipped tests can then be UNSKIPPED + assertions filled in without restructuring the spec files.

**Lab Engine + R-API-01 + R-UI-01 partial closure rationale:** business logic remains fully unit-tested (Phase 2.D 76 tests for Lab Engine, Phase 3.D 20 tests for /api/profile/certifications, Phase 4.D 16 tests for AnsiText). The PARTIAL closures here gap the BROWSER INTEGRATION layer specifically — exactly the layer E2E exists to cover. The gap is documented honestly rather than masked.

**Resolution (Wave 4B commit `<COMMIT_HASH_TBD>`):** Yol A locked. R-E2E-01/02/03 status = PARTIAL CLOSURE with explicit skip documentation. Phase 5 cycle CLOSED with honest scope acknowledgment.

---

**End of Phase 5.A audit. Wave 4B closed R-E2E-01/02/03 as PARTIAL per Z.13 Yol A. Z.8 + Z.10 + Z.12 inherited conventions permanently in effect.**
