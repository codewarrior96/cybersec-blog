...
| R-09 | Medium | A04 | identity-rules.ts | Reserved username list has only 3 entries (`ghost`, `analyst1`, `viewer1`); **broader sub-vector (A-05 closure Wave 1):** `USERNAME_RE = /^[a-zA-Z0-9_.-]{3,32}$/` has no constraint against consecutive dots — `a..b` is accepted. General repeat-character semantics in local-part is the broader pattern; T-IV06 captures the specific `..` case as a gap-test. | User registers `admin` / `root` / `support`. Audit logs and report attribution display authoritatively. Social engineering trivial. **Severity escalated from Low based on likelihood.** Consecutive-dot acceptance is a separate sub-vector under the same risk category (filesystem/DNS conflict if usernames are ever used as path segments). |
...
| R-16 | **Informational** *(A-11 closure Wave 1)* | A04 | middleware.ts | ~~Logout endpoint in PUBLIC_API_ROUTES; CSRF-able logout possible~~ **Corrected narrative (A-11 Wave 1):** logout route exports POST only (no GET handler); CSRF middleware fires on ALL mutations regardless of PUBLIC_API_ROUTES membership; cross-origin POST attempt → 403 'Origin mismatch'. PUBLIC_API_ROUTES bypass applies ONLY to `sessionPresenceCheck`, NOT `csrfCheck`. | Original attack vector (cross-site form auto-submit to /api/auth/logout) blocked at edge by CSRF gate. Residual surface: same-origin XSS only (script already running on victim.com can call `fetch('/api/auth/logout', ...)` → CSRF passes via matching Origin → session check bypassed by PUBLIC_API_ROUTES → logout succeeds). XSS preconditions are a separate (much larger) risk class; R-16's incremental contribution is informational. Severity downgraded Low → Informational per A-11 source-evidence. |
...
| R-22 ✅ ACCEPTED | Low | A07 | src/app/api/auth/login/route.ts (L83-97) | Login response code 403 with `EMAIL_NOT_VERIFIED` payload distinguishes "verified account, wrong password" (returns 401) from "unverified account, correct password" (returns 403 + email field). This response-shape divergence enables account-enumeration for accounts in the narrow unverified window between register and verify-click. The 403 also includes the user's email address (best-effort lookup via `readUserByUsername`; supabase store returns email, memory/postgres return null), adding a secondary leak when supabase-JSON store is active. | Source-code comments L76-82 document this as an intentional UX tradeoff: distinguishing "wrong password" from "unverified email" reveals that the username exists, but the alternative (silently failing an authenticated-but-unverified login) is confusing and pushes users toward password resets that won't help. **STATUS:** ACCEPTED — Wave 1 audit-doc-only closure per A-14 deferral. Severity Low because enumeration window is narrow (register→verify-click), and register-time emailKey uniqueness already provides equivalent enumeration signal (login parity here is internally consistent with register-time behavior). Future Phase 1.5 hardening cycle would mirror verify-resend invisible-resend pattern (generic 401 + silent verification-email resend on unverified accounts); T-LG03 already in place as the test guard that would flip from `403 EMAIL_NOT_VERIFIED` to `401 + audit-log invisible-resend` assertion on closure. **Reference:** A-14 in `pending-amendments.md` (full source-evidence + hardening proposal). |
...
| T-IR02 | Edge | `GHOST` (case) reserved | — *(A-01: happy-path, R-09 mapping removed Wave 1 — `normalizeIdentityUsername` applies trim+lowercase before reserved-set lookup)* |
| T-IR03 | Edge | ` ghost ` (whitespace) reserved | — *(A-01: happy-path, R-09 mapping removed Wave 1 — same normalization)* |
...
| T-AS09 | Exploit | `hasRoleAtLeast(unknownRole, validRequired)` returns false (defense-in-depth default; `ROLE_ORDER.indexOf(unknown) === -1`, -1 >= validIndex evaluates false) | — *(A-08 closure Wave 1: defensive-default OWASP A01-aligned guard, added in Phase 1.D.6)* |
