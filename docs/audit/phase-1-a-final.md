...
| R-19 ⏳ DEFERRED | High | A07 | soc-store-memory.ts (createSession, deleteSession), production fallback path | Memory store sessions are HMAC-signed self-contained tokens; revoked-tokens Set is instance-bound; logout fails distributed | If R-03 fallback activates: user clicks logout → token added to one instance's revokedTokens. Next request hits different instance → token still valid. Logout effectively no-op across cluster. **STATUS:** DEFERRED in Phase 1.5.6 (commit `4deb361`) — exploitability conditional on R-03 fallback activation. Steady-state production routes through Supabase store (distributed logout works correctly). R-19 reframes after R-03 closure: if R-03 fix removes memory fallback entirely, R-19 becomes inapplicable; if R-03 fix retains fallback with new policy, R-19 fix scope determined by that policy. Compound cluster analysis: see A-20 amendment. |
