...
import {
  listPosts, createPost, likePost, addComment,
  type PostCategory, type PostDifficulty,
...
export async function GET(request: NextRequest) {
  ...
  const posts = listPosts({ category: category ?? undefined, difficulty: difficulty ?? undefined, sort, limit });
  return NextResponse.json({ posts, total: posts.length });
}

/* POST — create post or action (like / comment) */
