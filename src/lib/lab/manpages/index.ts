...
const PAGES: Record<string, ManEntry> = {
  ...
  vim: {
    name: 'vim — Vi IMproved, modal text editor',
    synopsis: 'vim [OPTIONS] [FILE...]',
    description: 'Modal text editor with separate INSERT, NORMAL, VISUAL and COMMAND modes. Standard editor on most Linux/BSD systems and a common privilege-escalation surface (GTFOBins).',
    options: [
      ['i', 'Enter INSERT mode (NORMAL → INSERT)'],
      ...
      ['u / Ctrl+R', 'Undo / redo'],
      ['v / V', 'Character / line VISUAL select'],
    ],
    examples: [
      [':w', 'Write (save) file'],
      [':q', 'Quit'],
      ...
      [':!cmd', 'Run shell command without leaving vim'],
      [':set shell=/bin/bash', 'Change embedded shell (sudo vim → root shell via GTFOBins)'],
    ],
    seeAlso: ['vi(1)', 'nano(1)', 'sed(1)', 'ed(1)'],
  },
  sudo: {
