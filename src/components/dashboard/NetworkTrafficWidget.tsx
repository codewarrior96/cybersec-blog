...
function generateInitialData(): { time: string; inbound: number; outbound: number }[] {
  ...
  for (let h = 0; h < 24; h++) {
    for (let m = 0; m < 60; m += 15) {
      ...
      const hourFactor = Math.sin((h - 6) * Math.PI / 12) * 0.5 + 0.5;
      data.push({
        time: label,
...
export default function NetworkTrafficWidget() {
  ...
  if (data.length === 0) return <div className="absolute inset-0 bg-[#06000f]" />;
  ...
  return (
    <div className="absolute inset-0 flex flex-col overflow-hidden font-mono">
      ...
      <div className="flex justify-between items-center px-3 py-2 border-b border-violet-500/20 shrink-0">
        <span className="text-[var(--text-title)] font-bold text-violet-400 tracking-widest uppercase">⬡ SIGNAL SPECTRUM</span>
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5 text-[var(--text-body)]">
            <span className="w-3 h-[2px] bg-violet-400 rounded" />
            <span className="text-violet-400">INBOUND</span>
          </span>
          <span className="flex items-center gap-1.5 text-[var(--text-body)]">
            <span className="w-3 h-[2px] bg-orange-400 rounded" />
            <span className="text-orange-400">OUTBOUND</span>
          </span>
      ...
      <div className="flex-1 min-h-0 p-2 relative" ref={containerRef}>
        <svg width="100%" height="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="block">
          ...
          {yLabels.map(v => (
            <g key={v}>
              <line x1={padL} y1={toY(v)} x2={w} y2={toY(v)}
                stroke="rgba(139,92,246,0.09)" strokeWidth="0.8" />
              <text x={padL - 6} y={toY(v) + 3} textAnchor="end"
                fill="#475569" fontSize={isMobile ? '8' : '10'} fontFamily="monospace">
                {v}%
              </text>
            </g>
          ))}
          ...
          {xLabels.map((d, i) => (
            <text key={d.time}
              x={toX(isMobile ? i * 32 : i * 16)} y={h - 8}
              textAnchor="middle" fill="#475569"
              fontSize={isMobile ? '8' : '10'} fontFamily="monospace">
              {d.time}
            </text>
          ))}
          ...
          {/* Lines */}
          <path d={inboundPath}  fill="none" stroke="#8b5cf6" strokeWidth="2"   strokeLinejoin="round"
