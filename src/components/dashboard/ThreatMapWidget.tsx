...
export default function ThreatMapWidget({ attacks = [] }: ThreatMapWidgetProps) {
  ...
  return (
    <div className="absolute inset-0 flex flex-col font-mono overflow-hidden">
      ...
      {/* Header */}
      <div className="flex justify-between items-center px-3 py-2 border-b border-cyan-500/15 z-20 bg-var(--bg-panel)/80 shrink-0 touch-action-manipulation">
        <span className="text-[var(--text-title)] font-bold tracking-widest uppercase text-slate-300">// CYBER THREAT MAP</span>
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" /><span className="text-[var(--text-body)] text-red-400 font-bold">LIVE</span></span>
          <span className="text-slate-600 text-[var(--text-body)]">THREAT DETECTION</span>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 relative overflow-hidden">
        ...
        <div className="absolute top-3 right-3 z-20 text-right font-mono backdrop-blur-sm bg-var(--bg-panel)/70 p-2 rounded border border-cyan-500/20">
          <div className="text-[var(--text-body)] text-slate-400">Attack Speed: <span className="text-cyan-400 font-bold">23.50 MB/s</span></div>
          <div className="text-[var(--text-body)] text-slate-400">Volume: <span className="text-cyan-400 font-bold">123.2 MB/s</span></div>
        </div>

        {/* Bottom Left — LIVE THREAT DETECTION */}
        <div className="absolute bottom-4 left-4 right-4 sm:right-auto z-20 flex flex-col gap-1 bg-var(--bg-panel)/90 p-2 rounded border border-red-500/30 backdrop-blur-sm">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-6 h-6 rounded-full border border-red-500/80 flex items-center justify-center relative shadow-[0_0_10px_rgba(239,68,68,0.5)] bg-red-500/20 shrink-0">
              <div className="w-2 h-2 bg-red-500 rounded-full animate-ping absolute" />
              <div className="w-2 h-2 bg-red-500 rounded-full" />
            </div>
            <span className="text-[var(--text-title)] font-bold text-red-500 tracking-widest uppercase">LIVE THREATS</span>
          </div>
          <div className="flex flex-col gap-0.5">
            {attacks.length === 0 && (
              <span className="text-[var(--text-body)] text-slate-500 font-mono">Scanning grid...</span>
            )}
            {attacks.slice(-3).reverse().map((attack, i) => (
              <div key={i} className="text-[var(--text-body)] text-[var(--accent-cyan)] font-mono tracking-wider flex items-center gap-1.5 leading-tight truncate">
                <span className="w-1 h-1 bg-red-500 rounded-full animate-pulse shrink-0" />
                <span className="text-red-400 font-bold shrink-0">{attack.sourceIP}</span>
                <span className="text-slate-400 shrink-0">({attack.sourceCountry})</span>
              </div>
            ))}
          </div>
        </div>
